...
public class HienThiPhieuNhap extends FormHienThi {
    ...
    JComboBox<String> cbTypeSearch = new JComboBox<>(
            new String[] { "Tất cả", "Mã phiếu nhập", "Mã nhà cung cấp", "Mã nhân viên",
                    "Ngày lập", "Giờ lập", "Tổng tiền" });
    ...
    public HienThiPhieuNhap() {
        ...
        mtb = new MyTable();
        mtb.setHeaders(new String[] { "STT", "Mã phiếu nhập", "Mã nhà cung cấp", "Mã nhân viên", "Ngày nhập",
                "Giờ nhập", "Tổng tiền" });
        ...
        if (mapn != null) {
            ...
            for (PhieuNhap pn : qlpn.getDspn()) {
                if (pn.getMaPN().equals(mapn)) {
                    ...
                    txTongTien.setText(Format_money.format(pn.getTongTien()));
                    return;
                }
            }
        }
    ...
    private void txSearchOnChange() {
        ...
        setDataToTable(
                qlpn.search(cbTypeSearch.getSelectedItem().toString(), txTim.getText(), ngay1, ngay2, tong1, tong2),
                mtb);
    }
    ...
    private void btnDetailsMouseClicked() {
        ...
        if (i >= 0) {
            QuanLyChiTietPhieuNhapForm qlctpn = new QuanLyChiTietPhieuNhapForm(
                    mtb.getModel().getValueAt(i, 1).toString()); // truyen tham so cho qlctpn
    ...
    private void setDataToTable(ArrayList<PhieuNhap> data, MyTable table) {
        ...
        for (PhieuNhap pn : data) {
            table.addRow(new String[] {
                    ...
                    Format_money.format(pn.getTongTien())
            });
            stt++;
        }
    }
