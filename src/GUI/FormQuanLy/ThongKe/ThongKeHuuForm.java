...
class ThongKeSanPham extends JPanel {
    ...
    public ThongKeSanPham() {
        ...
        btnRefresh.setIcon(new ImageIcon(getClass().getResource("/images/icons8_data_backup_30px.png")));
    ...
    private void soLuongSanPhamNhap() {
        tb.clear();
        tb.setHeaders(new String[] { "Mã sản phẩm", "Tên sản phẩm", "Mã phiếu nhập", "Tên nhà cung cấp", "Ngày nhập",
                "Số lượng", "Ðơn giá", "Tổng tiền" });
        ...
        for (SanPham sp : qlspBUS.getDssp()) {
            ...
            tb.addRow(new String[] { "", "", "", "", mcd.getKhoangTG(), String.valueOf(tongSoLuong), "",
                    String.valueOf(tongTienPhieuNhapCuaMoiSanPham) });
            tb.addRow(new String[] { "", "", "", "", "", "", "", "" });
    ...
    private void soLuongSanPhamBan() {
        tb.clear();
        tb.setHeaders(new String[] { "Mã sản phẩm", "Tên sản phẩm", "Mã hóa don", "Tên nhân viên", "Ngày lập",
                "Số lượng", "Ðơn giá", "Tổng tiền" });
        ...
        for (SanPham sp : qlspBUS.getDssp()) {
            ...
            tb.addRow(new String[] { "", "", "", "", mcd.getKhoangTG(), String.valueOf(tongSoLuong), "",
                    String.valueOf(tongTienHoaDonTungSanPham) });
            tb.addRow(new String[] { "", "", "", "", "", "" });
            tongTatCa += tongSoLuong;
            tongTien += tongTienHoaDonTungSanPham;
        }

        tb.addRow(new String[] { "", "", "", "", "Tong tat ca", String.valueOf(tongTatCa), "",
                String.valueOf(tongTien) });
    }
            ...
            for (HoaDon hd : qlhdBUS.search("Tất cả", "", mcd.getNgayTu(), mcd.getNgayDen(), -1, -1)) {
                if (nv.getMaNV().equals(hd.getMaNhanVien())) {
                    tb.addRow(new String[] { "", "",
                            hd.getMaHoaDon(),
                            String.valueOf(hd.getNgayLap()),
                            Format_money.format(hd.getTongTien())
                    });
                    tongTien += hd.getTongTien();
                }
            }
            tb.addRow(new String[] { "", "", "", mcd.getKhoangTG(), Format_money.format(tongTien) });
            tb.addRow(new String[] { "", "", "", "", "", "" });

            tongTatCa += tongTien;
        }
        tb.addRow(new String[] { "", "", "", "Tổng tất cả", Format_money.format(tongTatCa) });
    }

    public void sanPhamCuaTungNhanVien_searchOnChange() {
        tb.setHeaders(new String[] { "Mã nhân viên", "Tên nhân viên", "Mã hóa đơn", "Ngày lập", "Mã sản phẩm",
        ...
        for (NhanVien nv : qlnvBUS.getDsnv()) {
            ...
            for (HoaDon hd : qlhdBUS.search("Mã nhân viên", nv.getMaNV(), mcd.getNgayTu(), mcd.getNgayDen(), -1, -1)) { // tương
                                                                                                                        // đối
                                                                                                                        // ->
                                                                                                                        // sai
                tb.addRow(new String[] { "", "", hd.getMaHoaDon(), String.valueOf(hd.getNgayLap()), "", "", "" });

                for (ChiTietHoaDon cthd : qlcthdBUS.search("Mã hóa đơn", hd.getMaHoaDon(), -1, -1, -1, -1)) { // tương
                                                                                                              // đối ->
                                                                                                              // sai
                    tongSoLuong += cthd.getSoLuong();
                    tb.addRow(new String[] { "", "", "", "",
            ...
            tb.addRow(new String[] { "", "", "", mcd.getKhoangTG(), "", "Tổng số sản phẩm",
...
class ThongKeKhachHang extends JPanel {
    ...
    public void sanPhamCuaTungKhachHang_searchOnChange() {
        tb.clear();
        tb.setHeaders(new String[] { "Mã khách hàng", "Tên khách hàng", "Mã hóa đơn", "Ngày lập", "Mã sản phẩm",
                "Tên sản phẩm", "Số lượng sản phẩm" });
        ...
        for (KhachHang kh : qlkhBUS.getDskh()) {
            ...
            for (HoaDon hd : qlhdBUS.search("Mã khách hàng", kh.getMaKH(), mcd.getNgayTu(), mcd.getNgayDen(), -1, -1)) { // tương
                                                                                                                         // đối
                                                                                                                         // ->
                                                                                                                         // sai
                tb.addRow(new String[] { "", "", hd.getMaHoaDon(), String.valueOf(hd.getNgayLap()), "", "", "" });
...
class ThongKeNhaCungCap extends JPanel {
    ...
    private void soLuongSanPhamCungCap() {
        tb.clear();
        tb.setHeaders(new String[] { "Mã nhà cung cấp", "Tên nhà cung cấp", "Mã phiếu nhập", "Ngày lập", "Mã sản phẩm",
        ...
        for (NhaCungCap ncc : qlnccBUS.getDsncc()) {
            ...
            tb.addRow(
                    new String[] { "", "", "", mcd.getKhoangTG(), "", "Tổng số lượng:", String.valueOf(tongSoLuong) });
            tb.addRow(new String[] { "", "", "", "", "", "", "" });
                ...
                if (pn.getMaNCC().equals(ncc.getMaNCC())) {
                    ...
                    for (ChiTietPhieuNhap ctpn : qlctpnBUS.search("Mã phiếu nhập", pn.getMaPN())) {
                        ...
                        tb.addRow(new String[] { "", "", "", "",
                                ...
                                String.valueOf(ctpn.getSoLuong()),
                                Format_money.format(ctpn.getSoLuong() * ctpn.getDonGia()) });
                    }
                }
            }
            tb.addRow(new String[] { "", "", "", mcd.getKhoangTG(), "", "", "Tổng thành tiền:",
