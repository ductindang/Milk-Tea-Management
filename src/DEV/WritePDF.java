...
public class WritePDF {
    ...
    public void writeHoaDon(String mahd) {
        ...
        try {
            ...
            Paragraph paraMaHoaDon = new Paragraph(
                    new Phrase("Hóa đơn: " + String.valueOf(hd.getMaHoaDon()), fontData));

            Paragraph para1 = new Paragraph();
            para1.setFont(fontData);
            para1.add(String.valueOf("Khách hàng: " +
            ...
            para2.add(String.valueOf(
                    "Nhân viên: " + qlnvBUS.getNhanVien(hd.getMaNhanVien()).getTenNV() + " - " +
                            hd.getMaNhanVien()));
            para2.add(glue);
            ...
            paraKhuyenMai.add("Khuyến mãi: " +
                    qlkmBUS.getKhuyenMai(hd.getMaKhuyenMai()).getTenKM());
            ...
            for (ChiTietHoaDon cthd : qlcthdBUS.getAllChiTiet(mahd)) {
                ...
                String gia = Format_money.format(cthd.getDonGia());
                String soluong = String.valueOf(cthd.getSoLuong());
                String thanhtien = Format_money.format(cthd.getDonGia() * cthd.getSoLuong());
    ...
    public void writePhieuNhap(String mapn) {
        ...
        try {
            ...
            Chunk glue = new Chunk(new VerticalPositionMark());// Khoang trong giua hang
            Paragraph paraMaHoaDon = new Paragraph(new Phrase("Phiếu nhập: " +
                    String.valueOf(pn.getMaPN()), fontData));
            Paragraph para1 = new Paragraph();
            para1.setFont(fontData);
            para1.add(String.valueOf(
                    "Nhà cung cấp: " + qlnccBUS.getNhaCungCap(pn.getMaNCC()).getTenNCC() + " - "
                            + pn.getMaNCC()));
            para1.add(glue);
            ...
            para2.setFont(fontData);
            para2.add(String
                    .valueOf("Nhân viên: " + qlnvBUS.getNhanVien(pn.getMaNV()).getTenNV() + " - "
                            + pn.getMaNV()));
            para2.add(glue);
            ...
            for (ChiTietPhieuNhap ctpn : qlctpnBUS.getAllChiTiet(mapn)) {
                ...
                pdfTable.addCell(new PdfPCell(new Phrase(Format_money.format(ctpn.getDonGia()), fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(String.valueOf(ctpn.getSoLuong()),
                        fontData)));
                pdfTable.addCell(new PdfPCell(
                        new Phrase(Format_money.format(ctpn.getDonGia() * ctpn.getSoLuong()),
                                fontData)));
            }
            ...
            Paragraph paraTongThanhToan = new Paragraph(
                    new Phrase("Tổng thanh toán: " + Format_money.format(pn.getTongTien()),
                            fontData));
            paraTongThanhToan.setIndentationLeft(300);
            document.add(paraTongThanhToan);
            document.close();
