...
public class QuanLyChiTietPhieuNhapDAO {
    ...
    public boolean add(ChiTietPhieuNhap ctpn) {
        qlctpnConnection = new ConnectionDB();
        Boolean ok = qlctpnConnection
                .sqlUpdate("INSERT INTO `chitietphieunhap`(`MaPN`,`MaSP`,`SoLuong`,`DonGia`) VALUE('"
    ...
    public Boolean delete(String _mapn, String _masp) {
        qlctpnConnection = new ConnectionDB();
        Boolean success = qlctpnConnection
                .sqlUpdate("DELETE FROM chitietphieunhap WHERE MaPN='" + _mapn + "' AND MaSP='" + _masp + "';");
        qlctpnConnection.closeConnect();
        return success;
    }
